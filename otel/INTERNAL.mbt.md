...
This document describes the OpenTelemetry bridge package. The package maps spans, events, fields, status, and links from `moonbit-community/tracing` to the `moonbit-community/opentelemetry` trace interface.

## Package Role

`otel` connects this repository's explicit `TraceContext` model to an existing OpenTelemetry SDK, exporter, or collector pipeline.

The package exposes three groups of capabilities:

- `with_otel_tracer` and `with_global_otel_tracer`: scoped bridge runtime helpers. They run the user callback and shut the runtime down.
...
`conversions.mbt` handles pure conversions between value domains: trace flags, trace state, span context, baggage, and tracing root context creation from an OpenTelemetry context.
...
OpenTelemetry span start, attribute update, event, status, and link operations run synchronously inside tracing subscriber callbacks. The background worker handles only `span.end()`.

The tracing hot path needs span state immediately. Later event, record, and link callbacks read that state. OpenTelemetry span end may trigger exporter work. The bridge sends span end through a bounded async queue.

`OtelRuntime::flush` waits until every already closed and successfully queued OpenTelemetry span has been ended. It does not close tracing spans that are still open.
...
`OtelSpanState` stores the OpenTelemetry `Span`, its corresponding `Context`, and the currently applied status. A `Ref` lets start, record, event, and end callbacks update the same span status.
...
3. Choose the OpenTelemetry span name. Use `otel.name` when present. Use the tracing metadata name as the fallback.
...
5. Append metadata attributes according to options. `target` is enabled by default. `level` and `loc` are disabled by default.
6. If `otel.kind` is present and valid, set the span kind.
7. Compute the parent context. Use the saved OpenTelemetry context for parent spans created by this runtime. Convert other tracing parents into OpenTelemetry span contexts.
...
Span kind takes effect only at start time. `otel.kind` on later span records is parsed and leaves the kind unchanged. This matches the OpenTelemetry span kind lifecycle.

## Span Record Mapping

`on_span_record` first checks whether the runtime is still accepting records. Records arriving after shutdown starts are counted as dropped and return before span lookup. During the accepting state, it looks up span state. A missing span state makes the record a no-op. This covers spans that have already ended and spans outside this runtime.
...
`otel.status_code` and `otel.status_description` can update status. Status updates follow the priority rule described below.

## Event Mapping

OpenTelemetry trace events must belong to a span. Spanless tracing events are not exported to OpenTelemetry. Spanless events that arrive during the accepting state are intentionally ignored by the bridge model and are not counted as dropped records.

An event inside a span calls `span.add_event(metadata.name, attributes)`. Event attributes include tracing fields, an optional `message` attribute, and target/level/location metadata according to options. `otel.*` fields on events are emitted as ordinary attributes.

Error-level events have two optional side effects controlled by options:

- `error_events_to_status`: mark the current span as error. The description uses the event message when present and the event name as the fallback.
- `error_events_to_exceptions`: call `record_error` to record an OpenTelemetry exception event. The message uses the tracing message when present and the event name as the fallback.

## Span End Mapping

`on_span_end` looks up span state. When it finds state, it removes the span from `span_states` and refreshes the unclosed span count. It then updates OpenTelemetry status based on tracing `SpanStatus`:
...
The span is then put into the end queue. The background worker calls `span.end()`. A non-accepting runtime or full end queue increments the drop counter.

## Status Priority

OpenTelemetry status updates use this priority order: `Unset < Error < Ok`.

`OtelSpanState::apply_status` calls `span.set_status` when the new status has a higher priority than the current status, or when both statuses have the same code. This gives the following behavior:
...
This rule lets callers explicitly control final status with `otel.status_code`. It also preserves default behavior from error events and error closes.

## Link Mapping

`on_span_link` converts tracing follows-from relationships into OpenTelemetry links.

For follows-from spans managed by this runtime, the bridge uses the saved OpenTelemetry span context. For other follows-from spans, it converts the tracing span context directly. Invalid contexts are not linked.

Links do not affect parentage. A span created by `spawn_follower_span` is not a child span in OpenTelemetry. It keeps the causal relationship through a link.
...
- `UInt64` becomes int64 when it fits in Int64. Larger values become decimal strings.
...
These fields are not emitted as ordinary span attributes by default. Control fields stay out of the attribute namespace.

## Background Worker

The worker calls `end()` on queued OpenTelemetry spans. It does not create spans, set attributes, or handle events.
...
The acknowledgement is currently `Ok(id)`. The OpenTelemetry span end interface used here does not expose writer-style errors. Closed control queues, closed acknowledgement queues, and timeouts still raise `OtelRuntimeError`.

## Runtime Control Semantics

`flush` sends `Flush(id)` during the running state and waits for a bounded acknowledgement. If shutdown is already in progress, flush waits for the shutdown acknowledgement. After successful shutdown, flush is a no-op.
...
Timeouts do not automatically mark the runtime as failed. After shutdown times out, the state may remain `ShuttingDown(id)`. A later call can continue waiting for the same acknowledgement.
...
`with_global_otel_tracer` obtains a tracer from the OpenTelemetry global provider and delegates to `with_otel_tracer`. Instrumentation name, version, schema URL, and attributes are forwarded to `@global.tracer`.
...
`baggage_from_otel` converts OpenTelemetry baggage back into tracing baggage. Entries rejected by tracing baggage validation are skipped.

`trace_context_from_otel` creates a tracing root context from an OpenTelemetry context. A valid span context becomes the tracing root's propagated parent. OpenTelemetry baggage is copied into the tracing context with or without a parent.

`OtelRuntime::current_context` converts in the other direction. It reads the current span from a tracing context. For spans created by this runtime, it returns the saved OpenTelemetry context. For propagated spans, it converts the propagated span into an OpenTelemetry context. Tracing baggage is always attached at the end.
...
`dropped_records` includes tracing callbacks that arrive after shutdown starts and span ends that cannot be enqueued due to a full end queue. Spanless events that arrive during the accepting state are intentionally ignored by the bridge model and are not counted as dropped.
